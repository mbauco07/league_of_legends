...
def Per_Level_Stats_output(_data, _championName):
    pls_table = PrettyTable()
    pls_table.title = 'Stats for: ' + _championName + ' at each level'
    Per_Level_Table_Helper_no_Dec(pls_table, 'Level' , 1, 1)
    Per_Level_Table_Helper_no_Dec(pls_table, 'Health', _data['hp'], _data['hpperlevel'])
    Per_Level_Table_Helper_no_Dec(pls_table, 'Mana/Resource', _data['mp'], _data['mpperlevel'])

    #are we dealing with Cassiopeia?
    if _championName != 'Cassiopeia':
        Per_Level_Table_Helper_no_Dec(pls_table, 'Move Speed', _data['movespeed'], 0)
    else: #we are dealing with Cassiopeia
        Per_Level_Table_Helper_no_Dec(pls_table, 'Move Speed', _data['movespeed']+ssv.cass_ms_per_level, ssv.cass_ms_per_level)

    Per_Level_Table_Helper_no_Dec(pls_table, 'Armor', _data['armor'], _data['armorperlevel'])
    Per_Level_Table_Helper_no_Dec(pls_table, 'Magic Resistance', _data['spellblock'], _data['spellblockperlevel'])

    #are we dealing with Tristana?
    if _championName != 'Tristana':
        Per_Level_Table_Helper_no_Dec(pls_table, 'Attack Range', _data['attackrange'], 0)
    else:
        Per_Level_Table_Helper_no_Dec(pls_table, 'Attack Range', _data['attackrange'], ssv.tristanna_range_per_level)

    Per_Level_Table_Helper_no_Dec(pls_table, 'HP Regeneration', _data['hpregen'], _data['hpregenperlevel'])
    Per_Level_Table_Helper_no_Dec(pls_table, 'Critical Strike', _data['crit'], _data['critperlevel'])
    Per_Level_Table_Helper_no_Dec(pls_table, 'Attack Damage', _data['attackdamage'], _data['attackdamageperlevel'])
    Per_Level_Table_Helper_2_Dec_Attack_Speed(pls_table, 'Attack Speed', round(_data['attackspeed'],2), _data['attackspeedperlevel'])
...
#Per_Level_Stats_output(championData[champion]['stats'], champion)
